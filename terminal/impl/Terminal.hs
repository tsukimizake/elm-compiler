...
app intro outro commands =
  do
    ...
    case argStrings of
      ["--dump-elmi", path] -> do
        maybeInterface <- File.readBinary path :: IO (Maybe Interface)
        case maybeInterface of
          Just interface ->
                print interface
          Nothing -> putStrLn $ "file: " <> path <> " not found!"
      [] ->
...
_maybeAutoComplete argStrings getSuggestions =
  if length argStrings /= 3
    then return ()
    else do
      maybeLine <- Env.lookupEnv "COMP_LINE"
      case maybeLine of
        Nothing ->
          return ()
        Just line ->
      ...
      let lo = snd (head chunk)
       ...
       in if point < lo
            then 0
              ...
              if point <= hi + 1
                then index
                else findIndex (index + 1) point cs
